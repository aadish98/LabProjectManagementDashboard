...
Security regressions cover exact loopback/state/PKCE handling, public-client token requests without a bundled secret, conditional consent, least scopes, initial/refreshed ID-token capture, revocation, local-storage token scrubbing, legacy migration, startup gating, awaited vault persistence, fresh-sign-in cleanup, and typed vault errors. `check:token-hygiene` scans source/configuration and generated frontend/backend bundles for the removed public-client secret variable, and scans unminified source for likely token logging and direct token `localStorage` writes; private `.env` files are intentionally neither read nor printed.
